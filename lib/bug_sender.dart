...
class _BugSenderPageState extends State<BugSenderPage> {
  ...
  // --- PERUBAHAN WARNA ---
  // Warna tema diubah dari ungu menjadi merah
  final Color primaryDark = Colors.black;
  final Color primaryWhite = Colors.white;
  final Color accentRed = Colors.redAccent; // Diubah dari ungu
  ...
  Future<void> _fetchSenders() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("http://dianaxyz-offc.hostingercloud.web.id:4278/mySender?key=${widget.sessionKey}"),
  ...
  void _showAddSenderDialog() {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(
          children: [
            Icon(Icons.add_circle, color: accentRed), // Diubah
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                ...
                labelStyle: TextStyle(color: accentRed), // Diubah
                hintText: "62xxx",
                hintStyle: TextStyle(color: Colors.white54),
                prefixIcon: Icon(Icons.phone, color: accentRed), // Diubah
                border: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(12),
                  borderSide: BorderSide(color: accentRed), // Diubah
                ),
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: accentRed, // Diubah
  ...
  Future<void> _addSender(String number, String name) async {
    ...
    try {
      final response = await http.get(
        Uri.parse("http://dianaxyz-offc.hostingercloud.web.id:4278/getPairing?key=${widget.sessionKey}&number=$number"),
      );
  ...
  void _showPairingCodeDialog(String number, String code, String name) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Column(
          children: [
            Icon(Icons.qr_code_2, color: accentRed, size: 50), // Diubah
        ...
        content: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: accentRed.withOpacity(0.3)), // Diubah
          ),
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: Colors.black,
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(color: accentRed), // Diubah
                ),
                child: Text(
                  code,
                  style: TextStyle(
                    color: accentRed, // Diubah
                    fontSize: 32,
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: accentRed), // Diubah
            onPressed: () {
              Navigator.pop(context);
              _fetchSenders();
            },
    ...
    if (confirmed == true) {
      ...
      try {
        ...
        final response = await http.delete(
          Uri.parse("http://dianaxyz-offc.hostingercloud.web.id:4278/deleteSender?key=${widget.sessionKey}&id=$senderId"),
        );
            ...
            Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.phone_android, color: accentRed), // Diubah
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(
                      foregroundColor: accentRed, // Diubah
                      side: BorderSide(color: accentRed), // Diubah
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.phone_iphone, color: accentRed, size: 80), // Diubah
          const SizedBox(height: 20),
          Text(
            "No Senders Found",
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: Icon(Icons.refresh, color: accentRed), // Diubah
            onPressed: isLoading ? null : _refreshSenders,
          ),
        ],
      ),
      body: isLoading && senderList.isEmpty
          ? const Center(child: CircularProgressIndicator(color: Colors.redAccent)) // Diubah
